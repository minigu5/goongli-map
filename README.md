...
대구과학고등학교 **궁리관**의 실험 용품을 빠르게 찾는 지도 웹사이트.
...
- **검색**: 상단 검색창에 이름·규격·카테고리·위치 입력 → 결과 클릭 시 해당 수납장으로 이동.
- **지도 탐색**: 휠로 확대/축소, 드래그로 이동. 초기 화면은 층 전체가 한눈에 보이는 크기로 시작. 오른쪽 아래 패널에서 층 전환.
- **수납장 보기**: 지도의 핀(층별 색상 직사각형)을 클릭하면 해당 수납장 용품 목록이 표시됨.
- **수정(로그인 필요)**: `@ts.hs.kr` 구글 계정 로그인 → **수정 모드** 진입.
  - **용품 추가**: 방 위를 클릭 → 마커·배너 표시 → **"+ 용품 추가"** 버튼으로 폼 열기.
    - 화장실·계단·복도는 클릭 불가.
    - 기존 수납장에 추가: 수납장 카드 하단 **"+ 이 위치에 용품 추가"** 클릭.
  - **수납장 이동**: 핀 클릭(노란 테두리) 후 드래그.
  - **층 순서 변경**: 수납장 카드 각 행 핸들(`⠿`) 드래그.
  - **용품 수정/삭제**: 수납장 카드의 수정·삭제 버튼 클릭.
  - **도형 데코 추가**: 오른쪽 위 **"+ 도형 추가하기"** 버튼 → 직사각형/원·색상 선택 → 지도 클릭으로 배치.
    - hover 시 꼭짓점 핸들로 크기 조절 가능.
