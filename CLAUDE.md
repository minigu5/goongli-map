...
- **렌더링**: `src/components/FloorMap.tsx` 가 좌표 데이터를 절대배치 `<div>` 로 그림.
  - 방 셀은 **빈 박스**로만 표시 (셀 내부 텍스트 없음). 외곽 테두리·존 라벨 없음.
  - 방 이름은 `MapView.tsx` 가 건물 **위(상단 행)·아래(하단 행) 외부 영역**에 표시.
    - 계단·화장실(service/stair) 라벨은 제외. 긴 이름은 2줄 자동 줄바꿈.
  - 텍스트는 컨테이너 쿼리 단위(`cqw`)로 지정 → 확대/축소 시 함께 스케일.
- 각 지도 위에 실험 용품을 **직사각형 핀**으로 표시한다(0~1 상대 좌표).
  - 핀 모양: 작은 직사각형 (기본 3×3 px). 과목별 연한 파스텔 색상으로 구분.
  - 핀 내부에 이름 최대 5글자를 매우 작은 폰트(`0.4cqw`)로 표시 → 줌인 시 읽힘.
  - 핀 크기(`pin_w`, `pin_h`, px 단위)는 Firestore에 저장되며 폼에서 편집 가능.
...
5. ✅ 핀 표시/추가/이동/수정/삭제 (수정 모드).
   - 핀 형태: 직사각형 (기본 3×3 px). 과목별 파스텔 색상.
   - 핀 내부에 이름 5글자 표시 (줌인 시 읽힘).
   - 핀 크기(`pin_w`/`pin_h`) 폼 편집 가능, Firestore 저장.
