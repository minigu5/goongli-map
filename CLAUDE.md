...
- **도형 추가**: 오른쪽 위 **"+ 도형 추가하기"** → 패널에서 직사각형/원·색상 선택 → 지도 빈 곳 클릭으로 배치.
  - **"도형 추가하기" 패널이 열린 상태에서만** 도형과 상호작용 가능.
  - hover 시 도형 전체 드래그로 이동, 꼭짓점 핸들(작은 흰색 사각형)로 리사이즈.
  - hover 시 오른쪽 위 × 버튼으로 삭제.
  - 일반 수정 모드(패널 닫힘)에서는 도형이 `pointer-events: none` → 도형 위 방 셀 클릭 시 용품 추가 가능.
...
10. ✅ 도형 데코레이션 기능.
    ...
    - hover 시 × 버튼으로 삭제.
11. ✅ 로직 개선.
    - **실시간 동기화**: `subscribeItems` / `subscribeDecorations` (Firestore `onSnapshot`) → 다른 사용자의 변경이 새로고침 없이 즉시 반영.
    - **도형 모드 분리**: 도형 드래그·리사이즈·삭제는 "도형 추가하기" 패널 열린 상태에서만 활성화 (`canInteract = editMode && !!decoMode`).
    - **도형 위 용품 추가**: 일반 수정 모드에서 도형 `pointer-events: none` → 도형 위 방 셀 클릭으로 용품 추가 가능.
    - **클릭/드래그 구분**: `FloorMap.onPointerDown`에서 좌표 기록 → `onClick`에서 5px 이상 이동 시 패닝으로 판단해 `onClickMap` 무시.
    - 수납장 생성 직후 `setSelectedCabinet` 직접 호출 (기존 `pendingCabinetPos` ref 패턴 제거).
